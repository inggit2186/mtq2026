<?php
$footerConfig = config('mtq.footer', []);
?>
<section class="mt-6">
        ...
        <div class="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)_320px] xl:items-start">
            <div class="space-y-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.28em] text-cyan-200">Didukung Oleh</p>
                        <p class="mt-1 text-sm text-slate-300">Identitas resmi pelaksana e-MTQ</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="flex h-20 items-center justify-center rounded-[1.35rem] border border-cyan-200/30 bg-slate-950/35 p-2.5 shadow-[0_18px_45px_-30px_rgba(125,211,252,0.35)] sm:h-24">
            ...
            <div class="min-w-0">
                ...
                <h2 class="mt-3 text-2xl font-black tracking-tight text-white sm:text-3xl"><?= e($footerConfig['headline'] ?? 'e-MTQ Kabupaten Tanah Datar') ?></h2>
                <p class="mt-4 max-w-3xl text-sm leading-7 text-slate-300 sm:text-base">
                    <?= e($footerConfig['description'] ?? 'Ruang kerja digital ini dihadirkan untuk membantu proses pendaftaran, verifikasi, penilaian, dan pengelolaan data MTQ secara lebih tertata, cepat, dan mudah dipantau oleh semua peran yang terlibat.') ?>
                </p>
                ...
                <div class="mt-5 flex flex-wrap items-center gap-3">
                    ...
                    <span class="status-pill border-cyan-400/18 bg-cyan-400/10 text-cyan-100">
                        <span class="inline-flex h-2.5 w-2.5 rounded-full bg-cyan-300"></span>
                        Data terhubung
                    </span>
                    <span class="status-pill border-amber-300/18 bg-amber-400/10 text-amber-100">
                        <span class="inline-flex h-2.5 w-2.5 rounded-full bg-amber-300"></span>
                        Siap operasional
                    </span>
                </div>

                <p class="mt-5 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm leading-7 text-slate-300">
                    ...
                    <strong class="text-white"><?= e($footerConfig['attribution_name'] ?? 'Ridho Saputra S.Kom') ?></strong>
                    <span class="text-cyan-100">- <?= e($footerConfig['attribution_role'] ?? 'Pranata Komputer Ahli Pertama - Kantor Kementerian Agama Kab. Tanah Datar') ?></span>
                </p>
            </div>

            <div class="rounded-[1.5rem] border border-white/10 bg-slate-950/30 px-5 py-4 shadow-[0_18px_45px_-30px_rgba(34,211,238,0.18)]">
                <p class="text-xs uppercase tracking-[0.22em] text-slate-400">Kontak & Informasi</p>
                <p class="mt-3 text-sm font-semibold text-white"><?= e($footerConfig['contact_label'] ?? 'CP WhatsApp') ?></p>
                <?php $contactNumber = preg_replace('/\D+/', '', (string) ($footerConfig['contact_number'] ?? '089509007078')); ?>
                <a href="https://wa.me/<?= e($contactNumber) ?>" target="_blank" rel="noreferrer" class="mt-2 inline-flex items-center gap-3 text-lg font-black tracking-[0.04em] text-cyan-200 transition hover:text-cyan-100">
                    ...
                    <span><?= e($footerConfig['contact_number'] ?? '0895-0900-7078') ?></span>
                </a>
                <p class="mt-3 text-xs leading-6 text-slate-400"><?= e($footerConfig['note'] ?? 'Dipakai sebagai sarana pendukung operasional e-MTQ dan komunikasi teknis pengembangan sistem.') ?></p>
            </div>
        </div>
    </div>
</section>
