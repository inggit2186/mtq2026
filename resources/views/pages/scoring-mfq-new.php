        ...
        <div class="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
            ...
            <div class="min-w-0 space-y-6">
                ...
                <?php if (!$activeSession && $selectedCategory):
                    $hasAnyData = false;
                    foreach ($completedSessions as $roundSessions) {
                        if (!$roundSessions->isEmpty()) { $hasAnyData = true; break; }
                    }
                ?>
                <section class="glass-card rounded-[2rem] p-6 mt-6" x-show="showStep >= 1">
                    <div class="flex items-start gap-4 mb-6">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="mt-2 text-sm text-slate-400">Rekap hasil sesi MFQ dan ranking berdasarkan total nilai.</p>
                        </div>
                    </div>
                    ...
                    <div x-data="{
                        ...
                        <?php foreach (['Penyisihan', 'Final'] as $round): ?>
                            <?php
                            $roundSessions = $completedSessions[$round] ?? collect();
                            $roundRankings = $rankingsData[$round] ?? collect();
                            ?>
                            <div x-show="activeTab === '<?= $round ?>'" class="space-y-6">
                                <?php if ($roundSessions->isNotEmpty()): ?>
                                ...
                                <div>
                                    ...
                                    <div class="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
                                        <?php foreach ($roundSessions as $session): ?>
                                ...
                                <!-- Rankings Table -->
                                <?php if ($roundRankings->isNotEmpty()): ?>
                                <div>
                                    <h4 class="text-sm font-semibold text-slate-300 mb-3 flex items-center gap-2">
